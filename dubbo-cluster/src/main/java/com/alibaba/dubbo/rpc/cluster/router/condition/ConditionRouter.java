...
public class ConditionRouter extends AbstractRouter {
    ...
    public ConditionRouter(URL url) {
        ...
        try {
            ...
            // 以=>为界，把规则分成两段，前面部分为whenRule,即消费者匹配条件;
            // 后面部分为thenRule,即提供者地址列表的过滤条件
            // 示例 method = find* => host = 192.168.1.22
            int i = rule.indexOf("=>");
            String whenRule = i < 0 ? null : rule.substring(0, i).trim();
            String thenRule = i < 0 ? rule.trim() : rule.substring(i + 2).trim();
            // 分别解析两个路由规则。调用parseRule方法，通过正则表达式不断循环匹配whenRule 和thenRule字符串。
    ...
    boolean matchWhen(URL url, Invocation invocation) {
        ...
        //     => host = 172.22.3.91
        // 表示所有的服务消费者可以调用 IP 为 172.22.3.91 的机器上的服务
        // >>>>>>>>> matchCondition 进行条件匹配
    ...
    private boolean matchCondition(Map<String, MatchPair> condition, URL url, URL param, Invocation invocation) {
        ...
        for (Map.Entry<String, MatchPair> matchPair : condition.entrySet()) {
            ...
            // 以上的代码实际上用于获取 sampleValue 的值，分割线以下才是进行条件匹配
            if (sampleValue != null) {
